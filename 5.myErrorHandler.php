...
function shutdown() {
    ...
    //не будем стирать сообщения об ошибках:
    if (is_array($error)) {
    ...
    if ($lineWithFatalError || !empty($linesWithWarningError)) {
        ...
        foreach ($fileArray as $num => $string) {
            ...
            if ($lineWithFatalError && ($num + 1) == $lineWithFatalError) {
                ...
                $backlightS = "<span style='background-color: #ff000030;'>";
                $backlightFin = "</span>";
            }
            //(?<!...) негативная ретроспективная проверка.
            //выводим код с подсветкой синтаксиса (highlight_string) и удаленными тэгами "<?php" (preg_replace)
            //и помечаем строки, которые вызвали ошибки. 
            echo  $backlightS.preg_replace("/(?<!\")&lt;\?php&nbsp;/", sprintf("%03d&nbsp;&nbsp;", $num + 1), highlight_string("<?php ".$string, true)).$backlightFin;
        } 
    } else {
